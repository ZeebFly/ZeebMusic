...
from ZeebMusic.misc import HAPP, SUDOERS, XCB, db
from ZeebMusic.utils import pastebin
...
from ZeebMusic.utils.decorators import AdminActual, asyncify, language
...
async def vardel_(client, message, _):
    ...
    if await is_heroku():
        ...
        if not output[0]:
            ...
            await message.reply_text(_["heroku_7"].format(check_var))
            os.system(f"kill -9 {os.getpid()} && python3 -m ZeebMusic")
