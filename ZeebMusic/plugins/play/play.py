...
from ZeebMusic.platforms import apple, resso, saavn, soundcloud
from ZeebMusic.platforms import spotify as spotifyapi
from ZeebMusic.platforms import telegram, youtube
from ZeebMusic.utils import seconds_to_min, time_to_seconds
from ZeebMusic.utils.database import is_video_allowed
from ZeebMusic.utils.decorators.play import PlayWrapper
from ZeebMusic.utils.formatters import formats
from ZeebMusic.utils.inline.play import (
    livestream_markup,
    playlist_markup,
    slider_markup,
    track_markup,
)
from ZeebMusic.utils.inline.playlist import botplaylist_markup
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await youtube.exists(url):
        ...
        elif await spotifyapi.valid(url):
            spotify = True
            ...
            if "track" in url:
                try:
                    details, track_id = await spotifyapi.track(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "youtube"
                img = details["thumb"]
                ...
                try:
                    details, plist_id = await spotifyapi.playlist(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "playlist"
                plist_type = "spplay"
                ...
                try:
                    details, plist_id = await spotifyapi.album(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "playlist"
                plist_type = "spalbum"
                ...
                try:
                    details, plist_id = await spotifyapi.artist(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "playlist"
                plist_type = "spartist"
