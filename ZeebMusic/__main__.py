...
async def init():
    ...
    await sudo()
    await app.start()
    for mod in app.load_plugins_from("ZeebMusic/plugins"):
    ...
    await userbot.start()
    await Yukki.start()
    LOGGER("ZeebMusic").info("Assistant Started Sucessfully")
    try:
    ...
    LOGGER("ZeebMusic").info("ZeebMusic Started Successfully")
    await idle()
    await app.stop()
    await userbot.stop()
    await Yukki.stop()
...
def main():
    ...
    LOGGER("ZeebMusic").info("Stopping ZeebMusic! GoodBye")
